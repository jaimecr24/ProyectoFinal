...
def getComments():
    ...
    comments = Comment.query.filter_by(idUser=current_user_id, idPlace=place)
    if comments is None:
        return jsonify({"count":0, "msg":"ok", "items":[]})
...
def add_comments():
    ...
    new_comment = Comment( time=datetime.now().date(), idPlace=comment_to_add["idPlace"], body=comment_to_add["body"], idUser=user.id )
